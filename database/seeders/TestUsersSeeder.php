...
class TestUsersSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Get a course from each institute (required – students need non-null course_id)
        $techCourse = $techInstitute ? Course::where('institute_id', $techInstitute->id)->first() : null;
        $paraCourse = $paraInstitute ? Course::where('institute_id', $paraInstitute->id)->first() : null;

        // Create Test Student for Tech Institute (only if institute and course exist)
        if ($techInstitute && $techCourse) {
            ...
            $this->command->warn('Skipped Tech test student: Tech institute or a course for it not found. Run InstituteSeeder and CourseSeeder first.');
        }

        // Create Test Student for Paramedical Institute (only if institute and course exist)
        if ($paraInstitute && $paraCourse) {
            Student::firstOrCreate(
            ...
            $this->command->warn('Skipped Paramedical test student: Paramedical institute or a course for it not found. Run InstituteSeeder and CourseSeeder first.');
        }
