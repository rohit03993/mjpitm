            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                @if($student->status === 'active' && $student->roll_number)
                    ...
                    @if(auth()->user() && auth()->user()->isSuperAdmin())
                        <a href="{{ route('admin.students.generate-semester-result', $student->id) }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded inline-flex items-center shadow-md">
                            <svg class="w-4 h-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div class="mb-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                @foreach($publishedSemesterResults as $semesterResult)
                                    @php
                                        // Double-check: Only show if truly published through proper workflow
                                        // Must have status='published', published_at, verified_at, AND verified_by
                                        $isPublished = $semesterResult->status === 'published' 
                                            && $semesterResult->published_at !== null 
                                            && $semesterResult->verified_at !== null
                                            && $semesterResult->verified_by !== null;
                                    @endphp
                                    @if($isPublished)
                                    <div class="border border-gray-200 rounded-lg p-4 bg-gray-50 hover:bg-gray-100 transition">
                                        ...
                                        <div class="flex gap-2">
                                            ...
                                            <a href="{{ route('admin.semester-results.download', $semesterResult->id) }}" 
                                               class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold py-2 px-3 rounded text-center transition inline-flex items-center justify-center">
                                                <svg class="w-4 h-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
